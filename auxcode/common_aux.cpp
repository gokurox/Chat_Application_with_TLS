...
void sigint_handler (int param)
{
	...
	for (fd_t fd = 3; fd <= MAX_OPEN_FD; fd++) {
		fd_t fd_to_close = fd;
		...
		case const_enums::CMD:
		{
			...
			switch (const_maps::CMD_TYPE_MAP[out_command[2]])
			{
				...
				case const_enums::CONNECT_RESPONSE:
				{
					...
					single_split (inp_message, delim, spos, epos, substring);	// ip_addr_len
					assert (epos != string_t::npos);
					skip_read_by_length (inp_message, spos, epos, substring);	// ip_addr
					out_command.push_back (substring);

					single_split (inp_message, delim, spos, epos, substring);	// port_len
					assert (epos != string_t::npos);
					skip_read_by_length (inp_message, spos, epos, substring);	// port
					out_command.push_back (substring);
